...
## 2026-09-02 - Correct cross-platform tracker hash enforcement

- Request: monitor the held-out seed job and continue through recoverable
  failures.
- Actions and evidence: retrieved the first private Kaggle worker log after it
  stopped; all 14 focused Kaggle tests passed, but the runner rejected the
  tracker file before inference. The recorded `ce957e...` hash was for the
  Windows CRLF checkout, while Git's canonical/Linux LF bytes hash to
  `951f0d...`. Recorded both representations and made the Kaggle runner enforce
  the canonical repository hash plus the freeze-manifest value.
- Engineering rationale: newline conversion changes raw bytes without changing
  YAML semantics. Naming both hashes avoids weakening provenance or falsely
  treating the same committed configuration as different on Linux.
- Result: the root cause is fixed without changing any detector or tracker
  parameter; the seed job is ready to relaunch.
- Limitation: no seed package was generated by the failed first run.
- Next: publish the correction and rerun the same frozen seed job.

## 2026-09-02 - Add a P100-compatible GPU probe path

- Request: keep monitoring and recover the private held-out seed generation.
- Actions and evidence: Kaggle version 2 passed 14 focused tests and every
  frozen hash check, then failed on its first inference frame. The worker log
  identified a Tesla P100 with compute capability 6.0, while the preinstalled
  PyTorch binary supported only capabilities 7.0-12.0. Added explicit GPU
  capability detection, a pinned PyTorch 2.5.1 CUDA 12.1 compatibility install
  for pre-7.0 GPUs, and a one-frame exact-pipeline probe before the full run.
- Engineering rationale: the failure is a binary/runtime incompatibility, not a
  detector-quality problem. A one-frame probe fails early and preserves the
  planned frozen inference settings.
- Result: the runner now handles Kaggle's P100 allocation deterministically and
  records the runtime in the delivery provenance.
